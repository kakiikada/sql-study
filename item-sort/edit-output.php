...
if(isset($_REQUEST['command'])){
  switch($_REQUEST['command']){
    ...
    case 'update':
      $sql = $pdo->prepare('update product set name=?, text=?, price=?, type=? where id=?');
      $sql->execute([$_REQUEST['name'],$_REQUEST['text'],$_REQUEST['price'],$_REQUEST['type'],$_REQUEST['id']]);
      break;

    case 'insert':
      $sql = $pdo->prepare('insert into product values(null,?,?,?,?)');
      $sql->execute([$_REQUEST['name'],$_REQUEST['text'],$_REQUEST['price'],$_REQUEST['type']]);
      break;
  }
}
//消去・更新・追加処理終わり
